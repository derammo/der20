...
function serialize(obj, prefix) {
    ...
    for (key in obj) {
        if (obj.hasOwnProperty(key)) {
            var queryVariable = prefix ? prefix + '[' + key + ']' : key;
            var value = obj[key];
            if (value !== null && typeof value === 'object') {
                // recurse
                var contents = serialize(value, queryVariable);
...
var url = "http://localhost:3000/character_log_imports/new?" + testing;
...
var longUrl = 'http://10.0.4.30/test?';
