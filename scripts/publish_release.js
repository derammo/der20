...
function getHistory(paths) {
    return execSync(`git log --date=short --format="##### %h by %an on %ad%n%w(0,4,4)%s%n%n%w(0,4,4)%N%b%n" $(git describe --tags --abbrev=0 HEAD~1)..HEAD -- ${paths}`, { encoding: 'utf-8' });    
}
...
for (let group of groups) {
    ...
    notes.push(getHistory(group));
}
notes.push('### Changes: Common')
notes.push(getHistory(common));const releaseSpec = {
    token: token,
    owner: 'derammo',
    repo: 'der20',
    tag: tag,
